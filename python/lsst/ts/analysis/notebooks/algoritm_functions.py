...
from lsst.ts.wep.Utility import DefocalType, getCamType
...
def plot_stamps(
    donutStamps, title="intra-focal", ncols=None, N=10, panel_width=4, indices=None
):
    # N = len(donutStamps)
    if indices is None:
        # take first N donuts
    ...
    fig, axs = plt.subplots(
        nrows, ncols, figsize=(ncols * panel_width, nrows * panel_width)
    )
    ax = np.ravel(axs)
    i = 0
...
def imageCoCenter_store(Img, inst, fov=3.5, debugLevel=0, store=None, increaseFactor=1.0):
    ...
    x1, y1 = Img._image.getCenterAndR()[0:2]
    ...
    fieldDist = Img._getFieldDistFromOrigin()
    ...
    I1c = Img.fieldX / fieldDist

    # Calculate the sin(theta) for projection
    I1s = Img.fieldY / fieldDist
    ...
    Img.updateImage(np.roll(Img.getImg(), int(np.round(stampCentery1 - y1)), axis=0))
    Img.updateImage(np.roll(Img.getImg(), int(np.round(stampCenterx1 - x1)), axis=1))
    store["cocenter_x_shift_amount"] = int(np.round(stampCentery1 - y1))
    store["cocenter_y_shift_amount"] = int(np.round(stampCenterx1 - x1))
    return store


def centerOnProjection_store(img, template, window=20, store=None):
...
def compensate_store(Img, inst, algo, zcCol, model, store):
    ...
    store["imgBeforeCompensate"] = Img.getImg()
    # Dimension of image
    sm, sn = Img.getImg().shape
    ...
    # Set up the mapping
    lutxp, lutyp, J = Img._aperture2image(
        inst, algo, zcCol, lutx, luty, projSamples, model
    )

    show_lutxyp = Img._showProjection(
    ...
    imgRecenter, store = centerOnProjection_store(
        Img.getImg(), show_lutxyp.astype(float), window=20, store=store
    )

    Img.updateImage(imgRecenter)
    ...
    ip = RectBivariateSpline(yp[:, 0], xp[0, :], Img.getImg(), kx=1, ky=1)
    ...
    Img.updateImage(lutIp * J)

    if Img.defocalType == DefocalType.Extra:
        Img.updateImage(np.rot90(Img.getImg(), k=2))
        # print('Rotated the extra-focal image')

    # Put NaN to be 0
    imgCompensate = Img.getImg()
    ...
    if np.any(imgCompensate < 0) and np.all(Img.image0 >= 0):
        print(
    ...
    Img.updateImage(imgCompensate)
    store["imgAfterCompensate"] = Img.getImg()
    return Img, store
...
def _solvePoissonEq_store(algo, I1, I2, iOutItr=0, store=None):
    ...
    if PoissonSolver == "fft":
        ...
        for jj in range(algo.getNumOfInnerItr()):
            ...
            offset = West[algo.mask_pupil == 1].mean()
            West = West - offset
            West[algo.mask_pupil == 0] = 0
        ...
        # Calculate dz
        focalLength = algo._inst.getFocalLength()  # focalLength
        offset = algo._inst.getDefocalDisOffset()  # defocalDisOffsetInM
...
def runIt_store(
    algo, I1, I2, model, tol=1e-3, store=None, doCoCenter=True, increaseFactor=1.0
):
    """
    ...
    while itr <= algo.getNumOfOuterItr():
        ...
        stopItr, store = _singleItr_store(
            algo, I1, I2, model, tol, store, doCoCenter, increaseFactor
        )
...
def _singleItr_store(
    ...
    if jj == 0:
        ...
        if doCoCenter:
            for Img, defocal in zip([I1, I2], ["intra", "extra"]):
                store[jj][defocal] = imageCoCenter_store(
                    Img,
                    algo._inst,
    ...
    if not algo.caustic:
        ...
        if compMode == "zer":
            ...
            I1, store[jj]["intra"] = compensate_store(
                I1, algo._inst, algo, algo.zcomp, model, store=store[jj]["intra"]
            )
            I2, store[jj]["extra"] = compensate_store(
                I2, algo._inst, algo, algo.zcomp, model, store=store[jj]["extra"]
            )
        ...
        algo.zc, algo.West, store[jj]["both"] = _solvePoissonEq_store(
            algo, I1, I2, jj, store[jj]["both"]
        )
    ...
    if jj > 0:
        diffZk = np.sum(np.abs(algo.converge[:, jj] - algo.converge[:, jj - 1])) * 1e9
...
def get_butler_stamps(
    ...
    donutStamps = butler.get(dataset_type, dataId=dataId, collections=[collection])

    donutCatalog = butler.get("donutCatalog", dataId=dataId, collections=[collection])
    return donutStamps, donutCatalog


def get_butler_image(
    repoDir, instrument="LSSTComCam", iterN=0, detector="R22_S01", collection=""
):
    butler = dafButler.Butler(repoDir)
    ...
    postIsr = butler.get("postISRCCD", dataId=dataId, collections=[collection])
    return postIsr
...
def get_comp_image(
    ...
    imgExtra.setImg(field_xy, DefocalType.Extra, image=z)
    imgExtra.makeMask(instrument, optical_model, boundaryT, maskScalingFactorLocal)

    return imgExtra


def plot_imageCoCenter(algo, store, I1, I2, I1imgInit, I2imgInit, I1shifts, I2shifts):
    ...
    for row, I, imgInit, shift in zip(
        [0, 1], [I1, I2], [I1imgInit, I2imgInit], [I1shifts, I2shifts]
    ):
        ...
        ax[row, 2].plot(
            [[stampCenter, stampCenter], [stampCenter + shift[0], stampCenter]],
        )
...
def plot_algo_steps(store, iterNum=0):
    ...
    ax[0, 0].text(
        0.8, 1.2, f"iteration {iterNum}", transform=ax[0, 0].transAxes, fontsize=19
    )
    for defocal, col in zip(["intra", "extra"], [0, 1]):
        print(defocal, col)
        row = 0
    ...
    for key in titles.keys():
        ax[row, 1].text(
            1.05, 0.5, titles[key], fontsize=19, transform=ax[row, 1].transAxes
        )
        row += 1
